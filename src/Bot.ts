...
import RolesPermissions from "@handlers/Discord/RolesPermissions";
import Voice from "@handlers/Discord/Voice/Voice";
import Moving from "@handlers/Discord/Moving";
import Streams from "@handlers/Twitch/Streams";
import AdminCommands from "@handlers/Discord/Admin/AdminCommands";
import Replies from "@handlers/Discord/Replies";
      ...
      try {
        ...
        if (msg.content.startsWith(this.config.prefix) && msg.author.bot === false) {
          ...
          if (this.validator.hasPermission(msg, global.permissions.common)) {
            const keyWord = msg.content.split(`${this.config.prefix}`)[1].trim().split(" ")[0];
